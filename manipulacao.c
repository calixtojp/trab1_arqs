...
void processaRegistros(ArqDados_t *arq_dados, ArqIndex_t *arq_index, InfoBusca_t *criterios, FncAcao acoes, FncFinaliza final){
    ...
    if(existe >= 0 ){
        ...
        busca_bin_index(arq_index,arq_dados,existe,criterios,acoes,final);
    }else{
        //se não, faz-se busca sequencial no arquivo de dados
        busca_seq_dados(arq_dados,arq_index,criterios,acoes,final);
    }
}
